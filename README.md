...
```sql
INSERT INTO
  readers (first_name, last_name, email, phone, address)
VALUES
...
```sql
INSERT INTO
  rents (reader_id, book_id, rent_date)
VALUES
...
### Все книги доступные для аренды
...
### Список жанров с количеством книг
...
GROUP BY
  b.book_id
ORDER BY
  COUNT(r.rent_id) DESC
LIMIT
  10;
```

### Читатели должники (держит книгу больше 30 дней)

```sql
SELECT
  r.last_name,
  r.first_name,
  MAX(DATEDIFF(CURRENT_DATE(), r2.rent_date)) AS days_rent
FROM
  readers r
  JOIN rents r2 ON r2.reader_id = r.reader_id
WHERE
  r2.return_date IS NULL
  and r2.rent_date < CURRENT_DATE() - INTERVAL 30 DAY
GROUP BY
  r.reader_id
ORDER BY
  days_rent DESC;
...
### Жанровые предпочтения читателей
...
FROM
  readers r
  JOIN rents r2 ON r2.reader_id = r.reader_id
  JOIN books b ON r2.book_id = b.book_id
